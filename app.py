...
def index():
    ...
    stats = {
        ...
        'total': 1972,
        'rev_table' : df2[['App','Category','Reviews', 'Rating']].groupby(['Category','App']).mean({'Reviews','Rating'}).round(2).sort_values('Reviews',ascending=False).head(10).reset_index().drop_duplicates('App',keep='first').to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }

    ## Bar Plot
    cat_order = df2.groupby('Category').agg({
    'Category' : 'count'
        }).rename({'Category':'Total'}, axis=1).sort_values('Total').head().reset_index()
